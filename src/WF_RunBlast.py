...
def main(argv):
	...
	if "BLAST_FINISHED" not in os.listdir(my_dir):
		...
		for c in args.children:
			...
			if len(strains) == 1:
				...
				for i in xrange(args.numThreads):
					blast_queue.put(Blast(["#BLAST_PATHblastp", "-outfmt", "6", "-evalue", str(args.blast_eval), "-qcov_hsp_perc", str(int(args.min_match_coverage * 100)), "-num_threads", "1", "-db", c_fasta, "-query", c_fasta + "." + "%04d" % (i), "-out", self_blast_out + "." + "%04d" % (i)]))
		...
		for i in xrange(args.numThreads):
			blast_queue.put(Blast(["#BLAST_PATHblastp", "-outfmt", "6", "-evalue", str(args.blast_eval), "-qcov_hsp_perc", str(int(args.min_match_coverage * 100)), "-num_threads", "1", "-db", fastas[0], "-query", fastas[1] + "." + "%04d" % (i), "-out", m8s[1] + "." + "%04d" % (i)]))
			blast_queue.put(Blast(["#BLAST_PATHblastp", "-outfmt", "6", "-evalue", str(args.blast_eval), "-qcov_hsp_perc", str(int(args.min_match_coverage * 100)), "-num_threads", "1", "-db", fastas[1], "-query", fastas[0] + "." + "%04d" % (i), "-out", m8s[0] + "." + "%04d" % (i)]))
