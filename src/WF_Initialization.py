...
class Tree:
	...
	def makeNodeFlowWorkflowControl(self, nodeTier, childToParent, working_dir):
		...
		for n in nodeTier[1]: #tier 1 is the first tier above the leaf nodes
			...
			while not curNode in all_proc_nodes:
				kids = []
				...
				set_cmd_count+=1
				next_cmd_id = "1."+ str(sets)+"."+str(set_cmd_count)
				if not curNode in childToParent:
		...
		for n in nodes:
			for m in nodes:
				if m[0] <= n[0]:
					continue
				min_row = min(m[0],n[0])
				if m[0] == min_row and m[5]==True:
					continue
				if n[0] ==min_row and n[5] == True:
					continue
				if abs(n[3]-m[3]) > 1:
					continue
