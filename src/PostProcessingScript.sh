...
cat genomes/locus_tag_file.txt | grep -ve "\t1$" | grep N_0 | awk '{print "#SYNERCLUST_PATHClusterPostProcessing.py genomes/ nodes/"$2"/locus_mappings.pkl "$3" -a '$1'"}' >> post_process_all.sh
